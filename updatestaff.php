	...
	<?php 
	   if($_SERVER["REQUEST_METHOD"] == "POST")
		         {  
					   ...
					   if($id != "")
					   {
							$staffud="SELECT *FROM staffdetails WHERE id='$id'";
							$staffudqry=mysqli_query($db,$staffud);
							$staffudqryres=mysqli_fetch_array($staffudqry,MYSQLI_ASSOC);
							echo'<form action="updatestaffin.php" method="post">';
							echo '<label>Staff ID:</label>';
							echo'<input type="text" name="sid" value="'.$staffudqryres['id'].'"/><br>';
							...
							echo '<label>Staff Position:</label>';
							echo'<input type="text" name="sposition" value="'.$staffudqryres['position'].'" disabled/><br>';
